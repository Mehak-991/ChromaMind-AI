...
class SyntheticDatasetGenerator:
    ...
    def simulate_mixture(self, ratios: List[float], bases: List[str]) -> Tuple[List[float], List[float]]:
    ...
    def validate_dataset(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        report = {
            ...
            "ratio_sum_check": float(np.abs(df[[f"ratio_{b}" for b in self.pigment_properties.keys()]].sum(axis=1) - 1.0).max())
        }
        return report
