...
export const Dashboard: React.FC = () => {
  ...
  const handleExportPDF = () => {
    ...
    const content = `ChromaMind AI Paint Formulation Report
======================================
Prediction ID: ${predictionResult.predictionId}
Target HEX: ${targetColor.hex.toUpperCase()}
Predicted HEX: ${predictionResult.predictedHex.toUpperCase()}
Delta E: ${predictionResult.deltaE.toFixed(2)}
Similarity: ${predictionResult.confidenceScore.toFixed(1)}%
Status: ${predictionResult.status}

Pigment Recipe:
--------------------------------------
${predictionResult.ratios.map(r => `- ${r.pigment}: ${(r.ratio * 100).toFixed(2)}% (${r.weightGrams.toFixed(2)}g)`).join('\n')}

======================================
Generated by ChromaMind AI Platform`;
    const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `ChromaMind_Formulation_${predictionResult.predictionId.slice(0, 8)}.txt`;
    link.click();
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      await historyService.saveRecipe({
        ...
        target_lab: targetColor.lab,
        base_colors_config: baseColors.map(c => ({ id: c.id, hex: c.hex, lab: c.lab, name: c.name })),
        ml_predicted_ratios: predictionResult.ratios.map(r => r.ratio),
        optimized_ratios: predictionResult.ratios.map(r => r.ratio),
        delta_e: predictionResult.deltaE,
        confidence_score: predictionResult.confidenceScore
      });
      alert("Recipe successfully saved to history!");
    } catch (err) {
      console.error(err);
      alert("Error saving recipe to backend.");
    }
  };
